...
class CleanRemovedProductsCommand extends Command
{
    ...
    private function deleteDocumentsFromIndex(iterable $getDeletedDocumentIds, OutputInterface $output): int
    {
        ...
        foreach ($getDeletedDocumentIds as $id) {
            $documentsIdsToRemove[] = $id;
            if (\count($documentsIdsToRemove) >= self::DEFAULT_BATCH_SIZE) {
                $this->productAndProductModelClient->bulkDelete($documentsIdsToRemove);
                $numberOfDeleteDocuments += \count($documentsIdsToRemove);
                $progressBar->advance(\count($documentsIdsToRemove));
                $documentsIdsToRemove = [];
            }
        }
        ...
        if (\count($documentsIdsToRemove) > 0) {
            $this->productAndProductModelClient->bulkDelete($documentsIdsToRemove);
            $numberOfDeleteDocuments += \count($documentsIdsToRemove);
        }
